...
export function calculateDiscount(price: number, quantity: number): number {
  ...
  if (quantity >= 50) return price * 0.1;
...
describe("POST /api/orders", () => {
  ...
  it("returns 400 for invalid order data", async () => {
    const response = await request(app).post("/api/orders").send({ items: [] }).expect(400);
...
describe("Order Flow Integration", () => {
  ...
  it("order creation updates inventory and notifies warehouse", async () => {
    ...
    const notifications = await db.notifications.findByOrderId(orderResponse.body.id);
